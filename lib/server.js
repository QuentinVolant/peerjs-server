...
app._initializeWSS = function(server) {
  ...
  this._wss = new WebSocketServer({ path: path, server: server, proxied: true});
...
// check if id is a moderator's id
app._checkModerator = function(id) {
    var isModerator = false;
    var idSplited = id.split("-");
    if(idSplited[0] == "moderator"){
        isModerator = true;
    }
    return isModerator;
}
...
app._configureWS = function(socket, key, id, token) {
  ...
  // if id is a moderator's id then checking of token is always accepted.        
  if(self._checkModerator(id)){
    client.token = token;   
  }
  ...
  if (token === client.token) {
    ...
    socket.send(JSON.stringify({ type: 'ID-TAKEN', payload: { msg: ', token 1 : '+token+', token 2 : '+client.token + ', id :'+id} }));
  ...
  socket.on('error', function() {
    self._log('Socket closed because error:', id);
    console.log('Socket closed because error:', id);
    if (client.socket == socket) {
      self._removePeer(key, id);
    }
  });
